...
class Observer
{
  ...
  public function __construct($player, PAC $PAC)
  {
    $this->Player                  = $player;
    $this->PlayerName              = $this->Player->getName();
    $this->Main                    = $PAC;
    $this->ClientID                = $player->getClientId();
    $this->Logger                  = $PAC->getServer()->getLogger();
    $this->Server                  = $PAC->getServer();
    $this->JoinCounter             = 0;
    $this->KickMessage             = "";
  ...
  public function PACIsOnGround($pp)
  {
  ...
  public function NotifyAdmins($message)
  {
    if($this->GetConfigEntry("Verbose"))
    {
      ...
      foreach ($this->Main->PlayerObservers as $observer)
      {
        ...
        if ($player != null and $this->Player->hasPermission("PAC.admin"))
        {
  ...
  public function PlayerQuit()
  {
    ...
    {
      $this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName is no longer watched...");
    }
  }
  ...
  public function PlayerJoin()
  {
    ...
    if ($this->GetConfigEntry("I-AM-WATCHING-YOU"))
    {
      $this->Player->sendMessage(TextFormat::ESCAPE."$this->Colorized"."[PAC] > $this->PlayerName, I am watching you ...");
    }
  }
  ...
  public function PlayerRejoin()
  {
    ...
    if ($this->GetConfigEntry("I-AM-WATCHING-YOU"))
    {
      $this->Player->sendMessage(TextFormat::ESCAPE."$this->Colorized"."[PAC] > $this->PlayerName, I am still watching you ...");
      $this->Logger->debug      (TextFormat::ESCAPE."$this->Colorized"."[PAC] > $this->PlayerName joined this server $this->JoinCounter times since server start");
    }
  }
  ...
  public function PlayerRegainHealth($event)
  {
    ...
    {
      if ($this->Player->hasPermission("PAC.regen")) return;
  ...
  public function CheckForceOP($event)
  {
    if ($this->GetConfigEntry("ForceOP"))
    {
      if ($this->Player->isOp())
      {
        ...
        {
          $event->setCancelled(true);
          $message = "[PAC] > %PLAYER% used ForceOP!";
          $reason = "[PAC] > ForceOP detected!";
          $this->NotifyAdmins($message);
          $this->KickPlayer($reason);
        }
      }
  ...
  {
    if ($this->Player->hasPermission("PAC.fly")) return;
    ...
    if ($this->GetConfigEntry("Speed") or $this->GetConfigEntry("Fly") or $this->GetConfigEntry("Glide"))
    {
      ...
      if ($tps > 0.0 and $this->prev_tick != -1.0)
      {
        ...
        {
          if (!$this->Player->hasPermission("PAC.speed"))
          {
            ...
            {
            # $this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName failed InArray!"); 
            # Anti Speed
    ...
    if (!$this->PACIsOnGround($this->Player))
    {
         ...
         and !in_array(Block::COBWEB              , $this->clipsurroundings ))
      {
        if ($this->y_pos_old > $this->y_pos_new)
        {
          ...
          {
            if (!$this->Player->hasPermission("PAC.glide"))
            {
  ...
  public function CheckNoClip($event)
  {
    ...
    {
      if ($this->Player->hasPermission("PAC.noclip")) return;
      $level    = $this->Player->getLevel();
      ...
      {
        #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > Not_In_Array, VL: $this->PlayerNoClipCounter");        
  ...
  public function OnPlayerGameModeChangeEvent($event)
  {
    ...
    {
      if ($this->Player->hasPermission("PAC.forcegamemode")) return;
  ...
  {
    #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > Player Swing Event: $this->PlayerName!");
  }
  ...
  public function PlayerHasDamaged($event)
  {
    ...
    #$this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > Kill Aura Counter: $this->PlayerKillAuraCounter     V2: $this->PlayerKillAuraV2Counter  Speed: $this->x_speed");
    #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName : consistency = $aimconsistency, VL= $this->PlayerKillAuraV2Counter");
    #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName : Trueangle = $trueangle");
    if ($this->Player->getGameMode() == 1 or $this->Player->getGameMode() == 3) return;
    // Kill Aura
    if ($this->GetConfigEntry("KillAura"))
    {
      if (!$this->Player->hasPermission("PAC.killaura"))
      {
        if ($is_damaged_entity_a_player)
        {
          ...
          #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > THD $this->PlayerName : hittime = $this->hs_hit_time");
          ...
          if ($distance_xz >= 0.5)
          {
            ...
            {
              #$this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > counter V2: $this->PlayerKillAuraV2Counter");
              # V1
              ...
              #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > counter V1: $this->PlayerKillAuraCounter  V2: $this->PlayerKillAuraV2Counter distance: $distance_xz  deltat: $delta_t  speedx: $this->x_speed anglexz: $angle_xz");
            }
            #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "AAA[PAC] > counter V1: $this->PlayerKillAuraCounter  V2: $this->PlayerKillAuraV2Counter distance: $distance_xz  deltat: $delta_t  speedx: $this->x_speed anglexz: $angle_xz");
          }  
    ...
    {
      if (!$this->Player->hasPermission("PAC.reach"))
      {
        $reach_distance = $damager_position->distance($damaged_entity_position); 
        #$this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > Reach distance $this->PlayerName : $reach_distance");
      ...
      if ($reach_distance > $this->GetConfigEntry("KickRange"))
      {
        ...
        #$this->Logger->debug(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName  ReachCounter: $this->PlayerReachCounter");
        $tick = (double)$this->Server->getTick(); 
        $tps  = (double)$this->Server->getTicksPerSecond();
  ...
  { 
    ...
    if ($this->GetConfigEntry("FastBow"))
    {
      ...
      if ($this->hs_arr_idx >= $this->hs_arr_size) $this->hs_arr_idx = 0;          
      $this->hs_hit_time = $this->hs_time_sum / $this->hs_arr_size;
      #$this->Logger->info(TextFormat::ESCAPE."$this->Colorized" . "[PAC] > $this->PlayerName : Shottime = $this->hs_hit_time");
